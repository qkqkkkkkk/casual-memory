...
def main() -> None:
    ...
    for recall in recalls:
        ...
        rows.append({"gold_recall": recall, "team_accuracy": correct / max(1, total), "individual_accuracy": individual_correct / max(1, 3 * total), "runs": total})
    ...
    if selected:
        ...
        with selected_path.open("x", encoding="utf-8") as handle:
            for claim in test: handle.write(json.dumps(make_variant(claim, bank, selected["gold_recall"], args.seed), ensure_ascii=False) + "\n")
