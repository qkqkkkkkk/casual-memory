...
from .run_experiment_e1 import load, retrieve, schema_coverage
...
def main() -> None:
    ...
    p.add_argument("--test", type=Path, required=True); p.add_argument("--experience-bank", type=Path, required=True); p.add_argument("--endpoint", default="http://127.0.0.1:11434/v1"); p.add_argument("--api-key", default=None); p.add_argument("--model", default="qwen2.5:7b"); p.add_argument("--top-k", type=int, choices=(1,), default=1); p.add_argument("--embedding-model", default="sentence-transformers/all-MiniLM-L6-v2"); p.add_argument("--retrieval-threshold", type=float, default=0.3); p.add_argument("--output-dir", type=Path, required=True)
    ...
    if schema_coverage(bank) != 1.0:
        raise SystemExit("Experience bank uses the old memory schema; rebuild it with the current build_pools command")
    index = GMemorySemanticIndex(bank, args.embedding_model, args.retrieval_threshold)
    shared = retrieve(claim, bank, "A1", args.top_k, index)
    ...
    report = {"pass": bool(same and second.llm_calls == 0 and second.cache_hits == 6), "semantic_outputs_identical": same, "second_run_llm_calls": second.llm_calls, "second_run_cache_hits": second.cache_hits, "expected_second_run_cache_hits": 6, "experience_bank": str(args.experience_bank), "embedding_model": args.embedding_model, "retrieval_threshold": args.retrieval_threshold}
