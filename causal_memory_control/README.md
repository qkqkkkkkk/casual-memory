...
The primary arm is receiver-level `DROP`. `PLACEBO` and `GLOBAL_DROP` are
optional. Every invocation supports `--resume` and writes each completed
branch immediately to `audit_runs.jsonl`.
